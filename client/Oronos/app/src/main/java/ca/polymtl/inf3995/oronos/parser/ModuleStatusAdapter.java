...
public class ModuleStatusAdapter extends BaseAdapter {
    ...
    private class PCBPair
    {
        ...
        private String      getPCBName()                      { return name; }
        private int         getNoSerial()                     { return noSerial; }
        private StatusOfPCB getStatus()                       { return status; }
        private void        setStatus(StatusOfPCB newStatus)  { status = newStatus; }
        private int         getLastNoMsg()                    {return lastNoMsg; }
        private void        setLastNoMsg(int newNoMsg)        { lastNoMsg = newNoMsg; }
        private DateTime    getLastTimeSeen()                 { return lastTimeSeen; }
        private void        setLastTimeSeen(DateTime newDate) { lastTimeSeen = newDate; }
    }
    ...
    private int findPositionOfPCB(String PCBname, int noSerial, int noMsg) {
        ...
        for (index = 0; index < PCBList.size(); index++) {
            if (PCBList.get(index).getNoSerial() == noSerial) { break; }
        }
        if (index == PCBList.size()) { addItem(PCBname, noSerial, noMsg); }
        return index;
    }
