...
class Base:
    ...
    def writerentry(self, nph, client, mode):
        ...
        while True:
            ...
            if ret == 0:
                self._alive = False        # signal the pipe has closed
                client._rq.put(None)       # signal the pipe has closed
                return
...
class ServerClient:
    def __init__(self, handle, mode, maxmessagesz):
        ...
        self._alive = True
        self._mode = mode
        self._maxmessagesz = maxmessagesz
        '''
    ...
    def read(self):
        ...
        if not self._alive and not self.canread():
            raise Exception('Pipe is dead!')
        if Mode.is_writer(self._mode):
    ...
    def write(self, message):
        if not self._alive:
            raise Exception('Pipe is dead!')
        if Mode.is_reader(self._mode):
            raise Exception('This pipe is in read mode!')
        if Mode.is_slave(self._mode) and not self.rlock.acquire(blocking = False):
            raise Exception('The pipe is currently being read!')
        if Mode.is_master(self._mode) and self.pendingread:
            raise Exception('Master mode must wait for slave reply!')

        self.pendingread = True
        self._wq.put(message)
        if Mode.is_slave(self._mode):
            self.rlock.release()
        return True

    def canread(self):
        return not self._rq.empty()

    def close(self):
        hk32['CloseHandle'](ctypes_handle(self._handle))
...
class Client(Base):
    def __init__(self, name, mode, *, maxmessagesz = 4096):
        ...
        self.client = ServerClient(self._handle, self._mode, self._maxmessagesz)

        if not Mode.is_writer(self._mode):
            thread = threading.Thread(target = self.readerentry, args = (self._handle, self.client, self._mode, None))
            thread.start()

        if not Mode.is_reader(self._mode):
            thread = threading.Thread(target = self.writerentry, args = (self._handle, self.client, self._mode))
            thread.start()
...
class Server(Base):
    def __init__(self, name, mode, *, maxclients = 5, maxmessagesz = 4096, maxtime = 100):
        ...
        self._maxmessagesz = 4096
        self._maxtime = maxtime
        self._shutdown = False
        self._server_thread = threading.Thread(target = self.serverentry)
    ...
    def dropdeadclients(self):
        ...
        for client in self._clients:
            if not client._alive and not client.canread():
    ...
    def serverentry(self):
        while not self._shutdown:
            ...
            nph = hk32['CreateNamedPipeA'](
                ctypes.c_char_p(b'\\\\.\\pipe\\' + bytes(self._name, 'utf8')),
                ctypes.c_uint(PIPE_ACCESS_DUPLEX),
                ctypes.c_uint(PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE),
                ctypes.c_uint(self._maxclients),
                ctypes.c_uint(self._maxmessagesz), ctypes.c_uint(self._maxmessagesz),
                ctypes.c_uint(self._maxtime),
                ctypes.c_uint(0)
            )
            ...
            client = ServerClient(nph, self._mode, self._maxmessagesz)

            if self._mode != Mode.Writer:
                thread = threading.Thread(target = self.readerentry, args = (nph, client, self._mode, self))
                thread.start()

            if self._mode != Mode.Reader:
                thread = threading.Thread(target = self.writerentry, args = (nph, client, self._mode))
                thread.start()
